...
Fband = 200;                                    % [Hz] 
Tlen = 1;                                       % [sec]
fanout = 3;                                     % nubmer of maximal peaks pairs in window
...
for i = 1:length(songs)
    ...
    if ~songFound
        ...
        [ peakMags, peaksN, F ,T ] = spectralFingerprint(dataS, fs, tRes, 100/overlapSize, gridSize);       % finds maximal peak magintude values
        [ dF, dT] = size2pixel(F,T,Fband,Tlen);                                                             % calculate peak-pairs window sizs
        [ hashlist ] = pairs2Hash(peaksN, dF, dT, fanout);                                                  % finds peak-pairs of song in form of indexs
